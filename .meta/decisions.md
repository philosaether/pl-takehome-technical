...
2026-06-27: **Built the one-pager** (`one-pager/`, committed e025086 + pushed). Toolchain: **LaTeX via tectonic** (self-contained, brew) + a py3.13 venv for matplotlib/scipy figures — chosen over MacTeX (4GB) and the broken py3.14 framework Python. Fig 3 manifold: **2-D filled-contour chosen** over 3-D (both rendered; 2-D reads in 20s, 3-D kept in figures/ for reference). Fig 1 left at the flattened aspect (works; squaring it overflowed to 2 pages). One-page discipline held throughout. Honcho dig (background agent) confirmed their `get_and_claim_work_units` IS our naive baseline → Fig 2 became the cited diff vs their real code; task-type cost profiles saved (webhook/reconciler are NOT Valkey candidates — surfaced + corrected before it shipped).

2026-06-27: **/review (feature/one-pager), expanded surface.** Fixes applied: (1) **top-level README rewritten** — was frozen at "M2 local" and said "154× at 10⁶" (contradicting the canonical 1,364× at 10⁷); now current, correct 5 subcommands, one-pager front-and-centre, no hyperlinks into `.meta/`. (2) Fig 3 manifold interp **cubic→linear** (cubic invented a smooth crossover the 4 process-rungs don't support). (3) Fig 1 labels **derived from CSV** (were hardcoded → desync risk). (4) tex/README nits + design-doc frontmatter reconciled.
- **Numbers honesty (judgment call):** the 26×/15× divide saturated Valkey by producer-bound (`saturated=false`) Postgres → upper bounds. Decision (Phil): **don't push harder** — copy already hedges "up to"/"as much as" + Methods discloses PG lower-bounds; Phil frames it verbally as proof-of-competence, "would rerun before a production call." Not a thesis. Skipped: masking unsaturated cells, the stock-vs-tuned-PG endnote, demoting 26×.
- **`.meta/` candor (judgment call):** README de-linked from `.meta/` so guests aren't routed into the audition/strategy notes, but `.meta/` stays **discoverable** (already in history; "honesty is the brand"; Phil is openly enthusiastic about the role). No history rewrite.
